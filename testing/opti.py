...
def history_records_api():
    files = [f for f in os.listdir(HISTORY_FOLDER) if f.endswith(".xlsx")]
...
def drop_employee():
    ...
    for index, emp in enumerate(employees, start=1):
        if emp["id_employee"] != index:
            cursor.execute("UPDATE opti SET id_employee=%s WHERE id_employee=%s",
                           (index, emp["id_employee"]))
...
def salary_table_api():
    ...
    for r in records:
        r["time_in"] = r["time_in"].timestamp()
        r["time_out"] = r["time_out"].timestamp() if r["time_out"] else None
